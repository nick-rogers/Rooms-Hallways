$(document).ready(function(){
  ...
  var theNum = 0;
  if(thisPage.charAt(0) == 0){
    theNum = thisPage.charAt(1);
  }
  else {
    theNum = 10;
  }
  ...
  for(var i = 0; i < 10; i++){
    ...
    var arrayChoice = imageArray[theNum-1];
    var image = arrayChoice[i];

    console.log(arrayChoice);

    //console.log('count ' + i);
    var randomizedTop = Math.round(Math.random()*40);
    var randomizedLeft = Math.round(Math.random()*80);
    if(i == 0 ){
      var theHref = 'e1.html';
    }
    else {
      var theHref = 'e'+(i+1)+".html";
    }
    ...
    div.style.marginLeft = randomizedLeft+"%";
    div.style.marginTop = randomizedTop+"%";
...
var imageArray = [e1Images,e2Images,e3Images,e4Images,e5Images,e6Images,e7Images,e8Images,e9Images,e10Images];
